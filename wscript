...
#		'materialsystem/defshaders', deferred shaders, unused in overcharged
		'shadereditor',
		'mathlib',
		'particles',
		'scenefilecache',
...
def options(opt):
	...
	grp.add_option('--enable-rtt-shadows', action = 'store_true', dest = 'RTT', default = False, help = '')
...
def configure(conf):
	...
	if conf.options.RTT:
		conf.env.append_unique('DEFINES', ['DYNAMIC_RTT_SHADOWS', 'ASW_PROJECTED_TEXTURES'] )
