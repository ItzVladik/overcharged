...
CBoneSetupMemoryPool_Hydra<Quaternion> g_QaternionPool_Hydra;
CBoneSetupMemoryPool_Hydra<Vector> g_VectorPool_Hydra;
CBoneSetupMemoryPool_Hydra<matrix3x4_t> g_MatrixPool_Hydra;
	...
	float cycle, int boneMask)
{
	...
	{
		matrix3x4_t *boneToWorld = g_MatrixPool_Hydra.Alloc();
		CBoneBitList_Hydra boneComputed;
		...
		g_MatrixPool_Hydra.Free(boneToWorld);
	}
}
	...
	int boneMask)
{
	...
	matrix3x4_t *srcBoneToWorld = g_MatrixPool_Hydra.Alloc();
	CBoneBitList_Hydra srcBoneComputed;

	matrix3x4_t *destBoneToWorld = g_MatrixPool_Hydra.Alloc();
	CBoneBitList_Hydra destBoneComputed;

	matrix3x4_t *targetBoneToWorld = g_MatrixPool_Hydra.Alloc();
	...
	g_MatrixPool_Hydra.Free(srcBoneToWorld);
	g_MatrixPool_Hydra.Free(destBoneToWorld);
	g_MatrixPool_Hydra.Free(targetBoneToWorld);
}
	...
	)
{
	...
	Vector		*pos2 = g_VectorPool_Hydra.Alloc();
	Quaternion	*q2 = g_QaternionPool_Hydra.Alloc();
	Vector		*pos3 = g_VectorPool_Hydra.Alloc();
	Quaternion	*q3 = g_QaternionPool_Hydra.Alloc();
	...
	g_VectorPool_Hydra.Free(pos2);
	g_QaternionPool_Hydra.Free(q2);
	g_VectorPool_Hydra.Free(pos3);
	g_QaternionPool_Hydra.Free(q3);
