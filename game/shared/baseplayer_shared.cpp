...
#ifdef CLIENT_DLL	// Overcharged 3rd person deathcam
#include "input.h"
#endif
...
void CBasePlayer::ItemPostFrame()
{
	...
	if (GetActiveWeapon() && m_hUseEntity == NULL && !GetVehicle())
	{
		//Wall check below
		GetActiveWeapon()->WallClip(ToBasePlayer(this));
	}
	...
	else
	{
		if ( GetActiveWeapon() && (!IsInAVehicle() || UsingStandardWeaponsInVehicle()) )
		{
...

			{
				...
				if (cvar->FindVar("oc_weapons_allow_inspect_animations")->GetBool())
					GetActiveWeapon()->CheckAdmireAnimations(ToBasePlayer(this));
			}
		}
	}
...
void CBasePlayer::PlayStepSound( Vector &vecOrigin, surfacedata_t *psurface, float fvol, bool force )
{
	...
	if (GetWaterLevel() == 1 || GetWaterLevel() == 2)
	{
#if !defined( CLIENT_DLL )
		Splash();
#endif
...
Vector CBasePlayer::Weapon_ShootPosition(int attachment)
{
	Vector newVector;

	CBaseViewModel *pViewModel = this->GetViewModel();
	if (!pViewModel)
		return newVector;

	if (pViewModel->GetAttachment(attachment, newVector))
		return newVector;

	return newVector;
}

Vector CBasePlayer::Weapon_ShootPosition(const char *attachment)
{
	Vector newVector;
	...
	if (GetViewModel()->GetAttachment(attachment, newVector))
		return newVector;
...
{
	//if (Q_strcmp(pWeapon->GetWpnData().WeaponType, "grenade") == 0)
	...
	if (pLastWeapon && pLastWeapon->IsIronSighted())
		pLastWeapon->OffIronSight();
	if (pLastWeapon && pLastWeapon->IsScopeSighted())
		pLastWeapon->OffScopeSight();
...
void CBasePlayer::ViewPunch( const QAngle &angleOffset )
{
	...
	m_Local.m_vecPunchAngleVel += angleOffset * 20.f;
}
...
void CBasePlayer::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
	...
	m_hRagdoll->SetModel("models/zombie/classic.mdl");
	m_hRagdoll->SetModelIndex(modelinfo->GetModelIndex(STRING(GetModelName())));
	...
	if (m_lifeState != LIFE_ALIVE && !IsObserver())
	{
		...
		if ( cl_fp_ragdoll.GetBool() && m_hRagdoll.Get() )
		{
			...
			C_ClientRagdoll *pRagdoll = dynamic_cast<C_ClientRagdoll*>(pEntity);
...
#ifdef CLIENT_DLL
		else if (!this->IsAlive() && (cvar->FindVar("oc_playerview_attach_deathcam")->GetInt() == 1))
		{
			CalcFirstPersonDeathView(eyeOrigin, eyeAngles, fov);
		}
#endif
		else
		{
