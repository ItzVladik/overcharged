...
//static CUtlVector<const char*> allWeapons;
//-----------------------------------------------------------------------------
...
bool CBaseCombatCharacter::Weapon_Switch( CBaseCombatWeapon *pWeapon, int viewmodelindex /*=0*/ ) 
{
	...
	if (pWeapon->GetWpnData().animData[pWeapon->m_bFireMode].FirstDrawAnimation != ACT_RESET)
	{
		if (pWeapon->allWeapons.HasElement(MAKE_STRING(pWeapon->GetClassname())))
		{
			pWeapon->m_bFirstEquip = false;
		}
		else
		{
			pWeapon->m_bFirstEquip = true;
			pWeapon->allWeapons.AddToTail(MAKE_STRING(pWeapon->GetClassname()));
		}
		//DevMsg("Weapon_Switch m_bFirstEquip: %i \n", pWeapon->m_bFirstEquip);
	}

	if (pWeapon->allObjectWeapons.HasElement(pWeapon))
	{
		pWeapon->m_bFireMode = pWeapon->allObjectWeapons.Element(pWeapon->allObjectWeapons.Find(pWeapon))->m_bFireMode;
		pWeapon->m_bFireModeCounter = pWeapon->allObjectWeapons.Element(pWeapon->allObjectWeapons.Find(pWeapon))->m_bFireModeCounter;
		//DevMsg("allObjectWeapons m_bFireMode %i \n", pWeapon->m_bFireMode);

		//pWeapon->m_bSilenced = pWeapon->allObjectWeapons.Element(pWeapon->allObjectWeapons.Find(pWeapon))->m_bSilenced;
	}

	if (pWeapon->GetWpnData().WeaponModes.Count() > 0 && !pWeapon->allObjectWeapons.HasElement(pWeapon))
	{
		pWeapon->m_bFireModesSize = pWeapon->GetWpnData().WeaponModes.Count();
		//pWeapon->m_bFireModeCounter = 0;
		if (pWeapon->GetWpnData().WeaponDefaultMode)
			pWeapon->m_bFireModeCounter = pWeapon->GetWpnData().WeaponModes.Find(pWeapon->GetWpnData().WeaponDefaultMode);
		else
			pWeapon->m_bFireModeCounter = 0;
		/*if (pWeapon->GetWpnData().WeaponModes.Count() == 1)
			DevMsg("WeaponModes 1: %.i \n", pWeapon->GetWpnData().WeaponModes.Element(0));
		else if (pWeapon->GetWpnData().WeaponModes.Count() == 2)
			DevMsg("WeaponModes 2: %.i %.i \n", pWeapon->GetWpnData().WeaponModes.Element(0), pWeapon->GetWpnData().WeaponModes.Element(1));
		else if (pWeapon->GetWpnData().WeaponModes.Count() == 3)
			DevMsg("WeaponModes 3: %.i %.i %i \n", pWeapon->GetWpnData().WeaponModes.Element(0), pWeapon->GetWpnData().WeaponModes.Element(1), pWeapon->GetWpnData().WeaponModes.Element(2));*/
	}

	if (pWeapon->GetWpnData().WeaponDefaultMode && !pWeapon->allObjectWeapons.HasElement(pWeapon))
	{
#ifndef CLIENT_DLL
		if (!pWeapon->HasSpawnFlags(FL_GRAPHED))
		{
			pWeapon->m_bFireMode = pWeapon->GetWpnData().WeaponDefaultMode;
			pWeapon->m_bFireModeCounter = pWeapon->GetWpnData().WeaponModes.Find(pWeapon->GetWpnData().WeaponDefaultMode);//pWeapon->GetWpnData().WeaponDefaultMode;
			pWeapon->burstFireCount = 0;
		}
		else
			pWeapon->RemoveSpawnFlags(FL_GRAPHED);
#endif

	}

	if (pWeapon->GetOwnerEntity())
	{
		switch (pWeapon->m_bFireMode)
		{
		case(1) :
#ifndef CLIENT_DLL
			//UTIL_HudHintText(pWeapon->GetOwnerEntity(), "Fire mode: semi-automatic");
			Q_strncpy(pWeapon->nameOfFireMode.GetForModify(), "FireModeType1", sizeof(pWeapon->nameOfFireMode));
#endif
			break;
		case(2) :
#ifndef CLIENT_DLL
			//UTIL_HudHintText(pWeapon->GetOwnerEntity(), "Fire mode: automatic");
			Q_strncpy(pWeapon->nameOfFireMode.GetForModify(), "FireModeType2", sizeof(pWeapon->nameOfFireMode));
#endif
			break;
		case(3) :
#ifndef CLIENT_DLL
			//UTIL_HudHintText(pWeapon->GetOwnerEntity(), "Fire mode: burst");
			Q_strncpy(pWeapon->nameOfFireMode.GetForModify(), "FireModeType3", sizeof(pWeapon->nameOfFireMode));
#endif
			break;
		default:
			break;
...
#ifndef CLIENT_DLL
	//if (Q_strcmp(pWeapon->GetWpnData().WeaponType, "annabelle") == 0)
	if (pWeapon->thisType == TYPE_ANNABELLE)
	{
		pWeapon->m_bFireMode = 0;
		pWeapon->m_bFireModeCounter = 0;
		pWeapon->burstFireCount = 0;
	}
#endif

	//DevMsg("m_bFireModeCounter equip: %.i \n", pWeapon->m_bFireModeCounter);
	// Already have it out?
	...
	/*if ((cvar->FindVar("oc_weapon_holster")->GetInt() == 1) && IsPlayer())	// BriJee OVR: Weapon holster anim.
	{
	}
	else*/
	{
	...
	if (pWeapon->m_bFireMode > 4 || pWeapon->m_bFireMode < 0)
		pWeapon->m_bFireMode = 0;

	if (cvar->FindVar("oc_weapons_allow_pickup_animations")->GetInt())
	{
		if (pWeapon->m_bFirstEquip && pWeapon->GetWpnData().animData[pWeapon->m_bFireMode].FirstDrawAnimation != ACT_RESET)
			return pWeapon->FirstDeploy(pWeapon->GetWpnData().animData[pWeapon->m_bFireMode].FirstDrawAnimation);
	}
...
bool CBaseCombatCharacter::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	...
	{
		CBasePlayer *pPlayer = (CBasePlayer *)this;
		if (pPlayer && pPlayer->GetActiveWeapon() && pPlayer->GetActiveWeapon()->IsIronSighted())
			pPlayer->GetActiveWeapon()->OffIronSight();
	}*/
...
bool CBaseCombatCharacter::Owner_HasFreeSlots(CBaseCombatWeapon* pWeapon)
{
	if (!cvar->FindVar("oc_player_weapons_limit")->GetInt())
		return true;

	int placesCount = cvar->FindVar("oc_player_weapons_limit")->GetInt();//7;

	for (int k = 0; k < MAX_WEAPONS; k++)
	{
		if (m_hMyWeapons[k].Get())
			placesCount--;
	}
	...
	int freePlaces[maxSlots][maxPositions] = { { 1, 2 }, { 1 }, { 1 }, { 1 }, { 1 }, { 1 } };
	...
	for (int i = 0; i < maxSlots; i++)
	{
		for (int j = 0; j < maxPositions; j++)
		{
			...
			for (int k = 0; k < MAX_WEAPONS; k++)
			{
				if (m_hMyWeapons[k].Get())
				{
					if (m_hMyWeapons[k].Get()->GetWpnData().iSlot == i)
					{
						if (m_hMyWeapons[k].Get()->GetWpnData().iPosition == j)
						{
							placesCount--;
						}
					}
