...
#if !defined( CLIENT_DLL )
#ifdef HL2_EPISODIC
	if (nEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT | EF_BATTERYLIGHT | EF_HEALTHLIGHT | EF_FRAGLIGHT | EF_FRAGSLIGHT | EF_FRAGFLIGHT | EF_EGONDISSOLVELIGHT))
	{
...
#if !defined( CLIENT_DLL )
#ifdef HL2_EPISODIC
	if (m_fEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT | EF_BATTERYLIGHT | EF_HEALTHLIGHT | EF_FRAGLIGHT | EF_FRAGSLIGHT | EF_FRAGFLIGHT | EF_EGONDISSOLVELIGHT))
	{
		// Hack for now, to avoid player emitting radius with his flashlight
		if ( !IsPlayer() )
		{
