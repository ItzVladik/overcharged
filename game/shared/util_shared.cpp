...
void UTIL_DecalTrace(const trace_t *pTrace, char const *decalName)
{
	if (pTrace->fraction == 1.0)
		return;
...
void UTIL_BloodImpact( const Vector &pos, const Vector &dir, int color, int amount )
{
	...
	DispatchParticleEffect("blood_impact_red_01", dir, vecAngles);*/
}
...
void UTIL_StringToIntUTILVector(CUtlVector<int> *pVector, const char *pString)
{
	pVector->RemoveAll();
	...
	for (int j = 0; j < 256; j++)
	{
		if (tempString[j] == NULL || tempString[j] == ' ')
		{

			int tmpf = atof(temp);
			if (/*!pVector->HasElement(tmpf) && */tmpf != 0)
				pVector->AddToTail(tmpf);
			//DevMsg("temp: %s \n", temp);
			//DevMsg("UTIL: temp[tempCount: %i]: %2.f \n", tempCount, tmpf);
		...
		//if (!pVector->HasElement(tempString[j] - '0'))
			//pVector->AddToTail(tempString[j] - '0');
		if (tempString[j] != ' ')
		{
			temp[tempCount] = tempString[j];
			tempCount++;
		}
	}
}
...
/*void UTIL_StringToIntUTILVector(CUtlVector<int> *pVector, const char *pString)
{
	...
	for (int i = 0; i < (int)Q_strlen(pString); i++)
	{
		if (pString[i] == NULL || pString[i] == ' ') continue;
		else
		{
			const char *elem = &pString[i];
			int value = atoi(elem);
			pVector->AddToTail(value);
		}
	}
}*/
void UTIL_StringToFloatUTILVector(CUtlVector<float> *pVector, const char *pString)
{
	pVector->RemoveAll();
	...
	for (int j = 0; j < 256; j++)
	{
		if (tempString[j] == NULL || tempString[j] == ' ')
		{
			...
			if (temp[0] == '-' || temp[1] == '-')
			{
				...
				tmpf = atof(temp);
			if (/*!pVector->HasElement(tmpf) && */tmpf != 0.0f)
			{
				pVector->AddToTail(tmpf);
				//DevMsg("pVector->Element(0): %.2f \n", pVector->Element(0));
			}
			//DevMsg("temp: %s \n", temp);
			//DevMsg("UTIL: temp[tempCount: %i]: %2.f \n", tempCount, tmpf);
