...
void C_TEExplosion::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	if ( UTIL_PointContents( m_vecOrigin ) & CONTENTS_WATER )
	{
		...
		{
			trace_t tr;
			int MAX_WATER_SURFACE_DISTANCE = 512;
			...
			VectorAngles(tr.endpos, tr.startpos, angOrigin);
			DispatchParticleEffect("water_splash_explosion", tr.startpos, angOrigin);
			...
			ep.m_pOrigin = &m_vecOrigin;
			C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, ep);

			EmitSound_t ep2;
			ep2.m_nChannel = CHAN_VOICE;
			ep2.m_pSoundName = "WaterExplosionEffect.Sound";
			ep2.m_flVolume = 1.0f;
			ep2.m_SoundLevel = SNDLVL_NORM;
			ep2.m_pOrigin = &m_vecOrigin;
			C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, ep2);
		}
		else
