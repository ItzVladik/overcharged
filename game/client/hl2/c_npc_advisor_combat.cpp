...
ConVar advisor_combat_elight_e("advisor_elight_e", "3");
ConVar advisor_combat_elight_rfeet("advisor_elight_rfeet", "52");
#endif


/*! Client-side reflection of the advisor class.
*/
class C_NPC_Advisor_combat : public C_AI_BaseNPC
{
	DECLARE_CLASS(C_NPC_Advisor_combat, C_AI_BaseNPC);
	DECLARE_CLIENTCLASS();
...
IMPLEMENT_CLIENTCLASS_DT(C_NPC_Advisor_combat, DT_NPC_Advisor_combat, CNPC_Advisor_combat)

END_RECV_TABLE()

// Server to client message received
void C_NPC_Advisor_combat::ReceiveMessage(int classID, bf_read &msg)
{
...
void C_NPC_Advisor_combat::ClientThink(void)
{
...
#if ADVISOR_ELIGHT_CVARS
		el->color.exponent = advisor_combat_elight_e.GetFloat();
		el->radius = advisor_combat_elight_rfeet.GetFloat() * 12.0f;
#endif
	}
}
...
//-----------------------------------------------------------------------------
void C_NPC_Advisor_combat::StartBeamFX(C_BaseEntity *pOnEntity)
{
	Assert(pOnEntity);
	if (!pOnEntity)
		return;
...
//-----------------------------------------------------------------------------
void C_NPC_Advisor_combat::StopBeamFX(C_BaseEntity *pOnEntity)
{
	Assert(pOnEntity);
	if (!pOnEntity)
		return;
...
void C_NPC_Advisor_combat::StartElight()
{
...
void C_NPC_Advisor_combat::StopElight()
{
