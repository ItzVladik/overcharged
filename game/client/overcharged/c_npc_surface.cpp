...
int C_NPC_Surface::DrawModel( int flags )
{
	...
	static SimpleParticle imp_particles[MAX_SURFACE_ELEMENTS + MAX_EXTRA_ELEMENTS];
	...
	if(sv_surface_testshape.GetBool())
	{
		for (int i = -10; i <= 10; i++)
		{
			SimpleParticle* imp_particle = &imp_particles[i + 10];
	...
	else
	{
		for (int i = 0 ; i < m_nActiveParticles; i++)
		{
			SimpleParticle* imp_particle = &imp_particles[i];
		...
		if(sv_surface_fountain.GetBool())
		{
			...
			for (int j = -7; j <= 7; j++)
			{
				SimpleParticle* imp_particle = &imp_particles[n_particles++];
