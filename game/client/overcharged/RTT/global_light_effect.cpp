...
void r_newGlightCallback_f(IConVar *pConVar, const char *pOldString, float flOldValue);

ConVar r_newglight("r_newglight", "1", FCVAR_CHEAT, "", r_newGlightCallback_f);
ConVar r_swingglight("r_swingglight", "1", FCVAR_CHEAT);
ConVar r_glightlockposition("r_glightlockposition", "0", FCVAR_CHEAT);
ConVar r_glightfov("r_glightfov", "45.0", FCVAR_CHEAT);
ConVar r_glightoffsetx("r_glightoffsetx", "10.0", FCVAR_CHEAT);
ConVar r_glightoffsety("r_glightoffsety", "-20.0", FCVAR_CHEAT);
ConVar r_glightoffsetz("r_glightoffsetz", "24.0", FCVAR_CHEAT);
ConVar r_glightnear("r_glightnear", "4.0", FCVAR_CHEAT);
ConVar r_glightfar("r_glightfar", "750.0", FCVAR_CHEAT);
ConVar r_glightconstant("r_glightconstant", "0.0", FCVAR_CHEAT);
ConVar r_glightlinear("r_glightlinear", "100.0", FCVAR_CHEAT);
ConVar r_glightquadratic("r_glightquadratic", "0.0", FCVAR_CHEAT);
ConVar r_glightvisualizetrace("r_glightvisualizetrace", "0", FCVAR_CHEAT);
ConVar r_glightambient("r_glightambient", "0.0", FCVAR_CHEAT);
ConVar r_glightshadowatten("r_glightshadowatten", "0.35", FCVAR_CHEAT);
ConVar r_glightladderdist("r_glightladderdist", "40.0", FCVAR_CHEAT);

#ifdef MAPBASE
ConVar r_glighttextureoverride("r_glighttextureoverride", "effects/glight", FCVAR_ARCHIVE);
#endif
...
//-----------------------------------------------------------------------------
CGlobalLightEffect::CGlobalLightEffect(int nEntIndex)
{
...
#ifdef MAPBASE
	if (r_glighttextureoverride.GetString()[0] != '\0')
	{
		m_FlashlightTexture.Init(r_glighttextureoverride.GetString(), TEXTURE_GROUP_OTHER, true);
	}
	else
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
CGlobalLightEffect::~CGlobalLightEffect()
{
	LightOff();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGlobalLightEffect::TurnOn()
{
	m_bIsOn = true;
	m_flDistMod = 1.0f;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGlobalLightEffect::TurnOff()
{
	if (m_bIsOn)
	{
		m_bIsOn = false;
...
//-----------------------------------------------------------------------------
void CGlobalLightEffect::UpdateLightNew(const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp)
{
	VPROF_BUDGET("CGlobalLightEffect::UpdateLightNew", VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
	...
	float flOffsetY = r_glightoffsety.GetFloat();

	if (r_swingglight.GetBool())
	{
	...
	Vector vTarget = vecPos + vecForward * r_glightfar.GetFloat();
	...
	if (r_glightvisualizetrace.GetBool() == true)
	{
	...
	if (flDist < flDistCutoff)
	{
		...
		float flPullBackDist = bPlayerOnLadder ? r_glightladderdist.GetFloat() : flDistCutoff - flDist;	// Fixed pull-back distance if on ladder
	...
	state.m_fQuadraticAtten = r_glightquadratic.GetFloat();
	...
	if (pPlayer)
	{
		...
		if (flBatteryPower <= 10.0f)
		{
			...
			if (flScale < 0.35f)
			{
				...
				{
					// On
					state.m_fLinearAtten = r_glightlinear.GetFloat() * flScale;
				}
				else
				{
					// Off
			...
			else
			{
				...
				state.m_fLinearAtten = r_glightlinear.GetFloat() * flScale + 1.5f * flNoise;
			}

			state.m_fHorizontalFOVDegrees = r_glightfov.GetFloat() - (16.0f * (1.0f - flScale));
			state.m_fVerticalFOVDegrees = r_glightfov.GetFloat() - (16.0f * (1.0f - flScale));
	...
	if (bFlicker == false)
	{
		state.m_fLinearAtten = r_glightlinear.GetFloat();
		state.m_fHorizontalFOVDegrees = r_glightfov.GetFloat();
		state.m_fVerticalFOVDegrees = r_glightfov.GetFloat();
	}

	state.m_fConstantAtten = r_glightconstant.GetFloat();
	state.m_Color[0] = 1.0f;
	state.m_Color[1] = 1.0f;
	state.m_Color[2] = 1.0f;
	state.m_Color[3] = r_glightambient.GetFloat();
	state.m_NearZ = r_glightnear.GetFloat() + m_flDistMod;	// Push near plane out so that we don't clip the world when the glight pulls back 
	state.m_FarZ = r_glightfar.GetFloat();
	...
	state.m_flShadowAtten = r_glightshadowatten.GetFloat();
	state.m_flShadowSlopeScaleDepthBias = cvar->FindVar("mat_slopescaledepthbias_shadowmap")->GetFloat();
	state.m_flShadowDepthBias = cvar->FindVar("mat_depthbias_shadowmap")->GetFloat();
	...
	{
		if (!r_glightlockposition.GetBool())
		{
...
// Purpose: Do the headlight
//-----------------------------------------------------------------------------
void CGlobalLightEffect::UpdateLightOld(const Vector &vecPos, const Vector &vecDir, int nDistance)
{
...
// Purpose: Do the headlight
//-----------------------------------------------------------------------------
void CGlobalLightEffect::UpdateLight(const Vector &vecPos, const Vector &vecDir, const Vector &vecRight, const Vector &vecUp, int nDistance)
{
	if (!m_bIsOn)
	{
		return;
	}
	if (r_newglight.GetBool())
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGlobalLightEffect::LightOffNew()
{
#ifndef NO_TOOLFRAMEWORK
	if (clienttools->IsInRecordingMode())
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGlobalLightEffect::LightOffOld()
{
	if (m_pPointLight && (m_pPointLight->key == m_nEntIndex))
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGlobalLightEffect::LightOff()
{
	LightOffOld();
	LightOffNew();
}
