...
static ConVar r_flashlightShadows("r_flashlightShadows", "0", FCVAR_ARCHIVE);
static ConVar r_newflashlight("r_newflashlight", "1", FCVAR_ARCHIVE, "", r_newflashlightCallback_f);
...
static ConVar r_flashlightfov( "r_flashlightfov", "45.0", FCVAR_ARCHIVE );
...
#endif
/*#ifdef MAPBASE
static ConVar r_flashlighttextureoverride( "r_flashlighttextureoverride", "effects/flashlight_border", FCVAR_ARCHIVE );
#endif*/
static ConVar r_flashlighttextureoverride("r_flashlighttextureoverride", "effects/flashlight_border", FCVAR_NONE);
...
//#endif // HL2_EPISODIC
	...
	state.m_bEnableShadows = r_flashlightShadows.GetBool();//r_flashlightdepthtexture.GetBool();
...
void CHeadlightEffect::UpdateLight( const Vector &vecPos, const Vector &vecDir, const Vector &vecRight, const Vector &vecUp, int nDistance, float vertFov, float horizFov )
{
	if ( IsOn() == false )
		 return;
	...
	state.m_fHorizontalFOVDegrees = horizFov;// 45.0f;
	state.m_fVerticalFOVDegrees = vertFov;// 30.0f;
	state.m_fQuadraticAtten = r_flashlightquadratic.GetFloat();
	state.m_fLinearAtten = r_flashlightlinear.GetFloat();
	state.m_fConstantAtten = r_flashlightconstant.GetFloat();
	state.m_Color[0] = 1.0f;
	...
	state.m_FarZ = r_flashlightfar.GetFloat();
	state.m_bEnableShadows = r_flashlightShadows.GetBool();//true;
	state.m_pSpotlightTexture = m_FlashlightTexture;
	state.m_nSpotlightTextureFrame = 0;
