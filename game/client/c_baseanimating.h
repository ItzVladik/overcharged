...
public:
	...
	void DispatchMuzzleHoldingEffect(C_BaseCombatWeapon *pWeapon);
	virtual bool DispatchMuzzleSecondaryEffect(const char *options, bool isFirstPerson);
	virtual bool DispatchMuzzleChargeEffect(const char *options);
	...
	float	SetNonThresholdPoseParameter(CStudioHdr *pStudioHdr, const char *szName, float flValue);
	inline float SetNonThresholdPoseParameter(const char *szName, float flValue) { return SetNonThresholdPoseParameter(GetModelPtr(), szName, flValue); }
	float	SetNonThresholdPoseParameter(CStudioHdr *pStudioHdr, int iParameter, float flValue);
	inline float SetNonThresholdPoseParameter(int iParameter, float flValue) { return SetNonThresholdPoseParameter(GetModelPtr(), iParameter, flValue); }
	...
	bool							CheckForcedRagdollName();
	virtual C_BaseAnimating			*BecomeRagdollOnClient();
	virtual C_BaseAnimating			*BecomeRagdollOnClientForced();
	bool							ForcedRagdollOnClient;
	C_BaseAnimating					*CreateRagdollCopy(bool forced = false);
	bool							InitAsClientRagdoll(const matrix3x4_t *pDeltaBones0, const matrix3x4_t *pDeltaBones1, const matrix3x4_t *pCurrentBonePosition, float boneDt, bool bFixedConstraints = false, bool forced = false);
	bool							InitAsClientRagdollDeath(const matrix3x4_t *pDeltaBones0, const matrix3x4_t *pDeltaBones1, const matrix3x4_t *pCurrentBonePosition, float boneDt, bool bFixedConstraints = false);
	...
	virtual void DoMuzzleFlashOVR(int attachmentIndex, int flashType, int r, int g, int b, const char *name);
