...
bool ShaderEditorHandler::Init()
{
	...
	bool bCreateEditor = (CommandLine() != NULL) && (CommandLine()->FindParm("-shadereditorui") != 0);
	bool bDisableEditor = (CommandLine() != NULL ) && ( CommandLine()->FindParm("-shadereditor_disable") != 0 );
	...
	if ( !bDisableEditor )
		shaderEditorModule = Sys_LoadModule("libshadereditor_2013" DLL_EXT_STRING );
	...
	{
		Warning("Cannot load shadereditor.dll from\n");
	}
...
void ShaderEditorHandler::LevelShutdownPreEntity()
{
}

void ShaderEditorHandler::SetMainViewMatrix( const VMatrix& view )
{
}
