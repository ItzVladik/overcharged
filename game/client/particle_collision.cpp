...
void CBaseSimpleCollision::TraceLine( const Vector &start, const Vector &end, trace_t *pTrace, bool coarse )
{
	...
	for ( int i = 0; i < m_nActivePlanes; i++ )
	{
		...
		{
			if (m_bDoDecal && m_iBloodColor >= 0)
				UTIL_TraceLine(start, end, MASK_ALL, NULL, COLLISION_GROUP_INTERACTIVE, pTrace);
			else
...
void CBaseSimpleCollision::TestForPlane( const Vector &start, const Vector &dir, float speed, float gravity )
{
	...
	for ( int i = 1; i <= NUM_DISCREET_STEPS; i++ )
	{
		...
		//Trace the line
		if (m_bDoDecal && m_iBloodColor >= 0)
			UTIL_TraceLine(testStart, testEnd, MASK_ALL, NULL, COLLISION_GROUP_INTERACTIVE, &tr);
		else
...
void CBaseSimpleCollision::DecalTraceLine(const Vector &start, const Vector &end, trace_t *pTrace, bool coarse, int color)
{
...
{	
	...
	if ( m_nActivePlanes > 0 )
	{
		/*if (m_bDoDecal)
		{
			...
			DecalTraceLine(origin, testPosition, pNewTrace, m_iBloodColor);
			//trace_t *pNewTrace = const_cast<trace_t*>(pTrace);
			//UTIL_DecalTrace(pNewTrace, "Blood");
		}
		else*/
		if (m_bDoDecal && m_iBloodColor >= 0)
			DecalTraceLine(origin, testPosition, pTrace, m_iBloodColor);
		else
		...
		if ( pTrace->fraction != 1.0f )
		{
			...
			if (m_bDoDecal && m_iBloodColor >= 0)
				UTIL_TraceLine(origin, testPosition, MASK_ALL, NULL, COLLISION_GROUP_INTERACTIVE, pTrace);
			else
			...
			{
				trace_t *pNewTrace = const_cast<trace_t*>(pTrace);
				const char *decalName = m_iBloodColor == 0 ? "Blood" : "YellowBlood";
				UTIL_DecalTrace(pNewTrace, decalName);
				
				surfacedata_t *pdata;
				pdata = physprops->GetSurfaceData(pTrace->surface.surfaceProps);
				CLocalPlayerFilter filter;
				//C_BaseEntity::EmitSound(filter, 0, "BloodDrips.Hit", pNewTrace->endpos);
				C_BaseEntity::EmitSound(filter, NULL, "BloodDrips.Hit", pdata->soundhandles.strainSound, &pNewTrace->endpos);
