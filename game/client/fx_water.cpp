...
void SplashCallback( const CEffectData &data )
{
	...
	if ( data.m_fFlags & FX_WATER_IN_SLIME )
	{
		...
		{
			DispatchParticleEffect("slime_splash_01", data.m_vOrigin, data.m_vAngles);
	...
	else
	{
		if (cvar->FindVar("oc_particle_impacts")->GetInt())
		{
			DispatchParticleEffect("water_splash_01", data.m_vOrigin, data.m_vAngles);
