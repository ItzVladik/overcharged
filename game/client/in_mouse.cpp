...
ConVar oc_player_zoom_sensitivity("oc_player_zoom_sensitivity", "1", FCVAR_ARCHIVE, "Player zoom sensitivity multiplicator");
...
void CInput::ScaleMouse( float *x, float *y )
{
	...
	if (pPlayer)
		mult = pPlayer->IsSighted() ? oc_player_zoom_sensitivity.GetFloat() : 1.f;
	//else
...
void CInput::MouseMove( CUserCmd *cmd )
{
		 ...
		 !vgui::surface()->IsCursorVisible() )
	{
		...
		bool isFreeAim = ::input->CAM_IsFreeAiming();

		if (!isFreeAim)
	    {
		     m_angViewAngle = viewangles;
	    }
		...
		if (isFreeAim)
		{
			...
			if (!pl->IsInAVehicle())// && pl->GetActiveWeapon())
			{
		...
		{
			ApplyMouse(viewangles, cmd, mouse_x, mouse_y);//ApplyMouse(m_angViewAngle, cmd, mouse_x, mouse_y);
			mouseX = (float)mouse_x;
			mouseY = (float)mouse_y;
		}
		// Re-center the mouse.
