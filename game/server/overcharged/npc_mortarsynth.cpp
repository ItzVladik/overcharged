...
// Interactions
//-----------------------------------------------------------------------------
int	g_interactionScannerInspect1 = 0;
int	g_interactionScannerInspect1Begin = 0;
int g_interactionScannerInspect1HandsUp = 0;
int g_interactionScannerInspect1ShowArmband = 0;//<<TEMP>>still to be completed
int	g_interactionScannerInspect1Done = 0;
int g_interactionScannerSupport1Entity = 0;
int g_interactionScannerSupportPosition1 = 0;
	...
	if (m_pSquad)
	{
		...
		for (CAI_BaseNPC *pSquadMember = m_pSquad->GetFirstMember(&iter); pSquadMember; pSquadMember = m_pSquad->GetNextMember(&iter))
		{
			if (pSquadMember != this)
			{
				if (GetTarget())
				{
					pSquadMember->DispatchInteraction(g_interactionScannerSupport1Entity, ((void *)((CBaseEntity*)GetTarget())), this);
				}
				else
				{
					pSquadMember->DispatchInteraction(g_interactionScannerSupportPosition1, ((void *)m_vInspectPos.Base()), this);
				}
			}
		}
	}
...
void CNPC_MSynth::TakePhoto(void)
{
	...
	if (GetTarget())
	{
		...
		if (GetTarget()->MyNPCPointer() != NULL)
		{
			...
			GetTarget()->MyNPCPointer()->DispatchInteraction(g_interactionScannerInspect1Begin, NULL, this);
		}
	}
...
void CNPC_MSynth::BlindFlashTarget(CBaseEntity *pTarget)
{
	...
	if (dotPr > 0.5f)
	{
		...
		{
			color32 white = { 255, 255, 255, (byte)(MSYNTH_FLASH_MAX_VALUE * dotPr) };
...
void CNPC_MSynth::StartTask(const Task_t *pTask)
{
	...
	case TASK_CSCANNER_SPOT_INSPECT_ON:
	{
		...
		else
		{
			...
			{
				pNPC->DispatchInteraction(g_interactionScannerInspect1Begin, NULL, this);
	...
	case TASK_CSCANNER_SPOT_INSPECT_WAIT:
	{
		...
		else
		{
			...
			else
			{
				...
				pNPC->DispatchInteraction(g_interactionScannerInspect1HandsUp, NULL, this);
			}
			TaskComplete();
		}
		break;
	...
	case TASK_CSCANNER_SPOT_INSPECT_OFF:
	{
		...
		else
		{
			...
			{
				pNPC->DispatchInteraction(g_interactionScannerInspect1Done, NULL, this);
...
bool CNPC_MSynth::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* pSourceEnt)
{
	...
	if (interactionType == g_interactionScannerSupport1Entity)
	{
	...
	else if (interactionType == g_interactionScannerSupportPosition1)
	{
		// Only accept help request if I'm not already busy
		if (GetEnemy() == NULL && !HaveInspectTarget())
		{
...
DECLARE_INTERACTION(g_interactionScannerInspect1)
DECLARE_INTERACTION(g_interactionScannerInspect1Begin)
DECLARE_INTERACTION(g_interactionScannerInspect1Done)
DECLARE_INTERACTION(g_interactionScannerInspect1HandsUp)
DECLARE_INTERACTION(g_interactionScannerInspect1ShowArmband)
DECLARE_INTERACTION(g_interactionScannerSupport1Entity)
DECLARE_INTERACTION(g_interactionScannerSupportPosition1)
