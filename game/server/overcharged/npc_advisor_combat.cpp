...
// Skill settings.
//
ConVar sk_advisor_combat_health("sk_advisor_combat_health", "0");
ConVar advisor_combat_use_impact_table("advisor_combat_use_impact_table", "1", FCVAR_NONE, "If true, advisor will use her custom impact damage table.");

ConVar sk_advisor_combat_melee_dmg("sk_advisor_combat_melee_dmg", "0");

#if NPC_ADVISOR_COMBAT_HAS_BEHAVIOR
ConVar advisor_combat_throw_velocity("advisor_combat_throw_velocity", "1100");
ConVar advisor_combat_throw_rate("advisor_combat_throw_rate", "4");					// Throw an object every 4 seconds.
ConVar advisor_combat_throw_warn_time("advisor_combat_throw_warn_time", "1.0");		// Warn players one second before throwing an object.
ConVar advisor_combat_throw_lead_prefetch_time("advisor_combat_throw_lead_prefetch_time", "0.66", FCVAR_NONE, "Save off the player's velocity this many seconds before throwing.");
ConVar advisor_combat_throw_stage_distance("advisor_combat_throw_stage_distance", "180.0", FCVAR_NONE, "Advisor will try to hold an object this far in front of him just before throwing it at you. Small values will clobber the shield and be very bad.");
//ConVar advisor_staging_num("advisor_staging_num","1",FCVAR_NONE,"Advisor will queue up this many objects to throw at Gordon.");
ConVar advisor_combat_throw_clearout_vel("advisor_combat_throw_clearout_vel", "200", FCVAR_NONE, "TEMP: velocity with which advisor clears things out of a throwable's way");
...
class CNPC_Advisor_combat;


//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
class CAdvisorCombatLevitate : public IMotionEvent
{
...
BEGIN_SIMPLE_DATADESC(CAdvisorCombatLevitate)
...
// The advisor class.
//-----------------------------------------------------------------------------
class CNPC_Advisor_combat : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_Advisor_combat, CAI_BaseNPC);
...
protected:
	...
	CAdvisorCombatLevitate m_levitateCallback;
...
LINK_ENTITY_TO_CLASS(npc_advisor_combat, CNPC_Advisor_combat);

BEGIN_DATADESC(CNPC_Advisor_combat)
...
DEFINE_ARRAY(m_haRecentlyThrownObjects, FIELD_EHANDLE, CNPC_Advisor_combat::kMaxThrownObjectsTracked),
DEFINE_ARRAY(m_flaRecentlyThrownObjectTimes, FIELD_TIME, CNPC_Advisor_combat::kMaxThrownObjectsTracked),
...
#if NPC_ADVISOR_COMBAT_HAS_BEHAVIOR
IMPLEMENT_SERVERCLASS_ST(CNPC_Advisor_combat, DT_NPC_Advisor_combat)
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::Spawn()
{
	BaseClass::Spawn();
	...
	m_iHealth = sk_advisor_combat_health.GetFloat();
...
void CNPC_Advisor_combat::Activate()
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::UpdateOnRemove()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::OnRestore()
{
	BaseClass::OnRestore();
...
//-----------------------------------------------------------------------------
Class_T	CNPC_Advisor_combat::Classify()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Advisor_combat::IsHeavyDamage(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::StartLevitatingObjects()
{
	if (!m_pLevitateController)
	{
...
bool CNPC_Advisor_combat::CanLevitateEntity(CBaseEntity *pEntity, int minMass, int maxMass)
{
...
CBaseEntity *CNPC_Advisor_combat::ThrowObjectPrepare()
{
	...
	if (pThrowable)
	{
		...
		m_flThrowPhysicsTime = gpGlobals->curtime + advisor_combat_throw_warn_time.GetFloat();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	...
	case TASK_ADVISOR_FIND_OBJECTS:
	{
		...
		class CAdvisorCombatLevitateEntitiesEnum : public CFlaggedEntitiesEnum
		{
		public:
		CAdvisorCombatLevitateEntitiesEnum( CBaseEntity **pList, int listMax, int nMinMass, int nMaxMass )
		...
		CAdvisorCombatLevitateEntitiesEnum levitateEnum( list, ARRAYSIZE( list ), 10, 220 );
	...
	case TASK_ADVISOR_LEVITATE_OBJECTS:
	{
		...
		m_flThrowPhysicsTime = gpGlobals->curtime + advisor_combat_throw_rate.GetFloat();
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::RunTask(const Task_t *pTask)
{
	...
	case TASK_ADVISOR_BARRAGE_OBJECTS:
	{
		...
		if ((gpGlobals->curtime > m_flThrowPhysicsTime - advisor_combat_throw_lead_prefetch_time.GetFloat()) &&
...
//----------------------------------------------------------------------------------------------
void CNPC_Advisor_combat::Event_Killed(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_Advisor_combat::PickThrowable(bool bRequireInView)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::HurlObjectAtPlayer(CBaseEntity *pEnt, const Vector &leadVel)
{
	IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
	...
	float flVelocity = advisor_combat_throw_velocity.GetFloat();
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::PreHurlClearTheWay(CBaseEntity *pThrowable, const Vector &toPos)
{
	...
	for (int i = 0; i < nFoundCt; ++i)
	{
			...
			!DidThrow(obstruction))						// and I didn't just throw it
		{
			...
			{
				thrust *= advisor_combat_throw_clearout_vel.GetFloat() / thrustLen;
			}
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::PurgeThrownObjects()
{
...
//-----------------------------------------------------------------------------
bool CNPC_Advisor_combat::DidThrow(const CBaseEntity *pEnt)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::AddToThrownObjects(CBaseEntity *pEnt)
{
	Assert(pEnt);
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::PullObjectToStaging(CBaseEntity *pEnt, const Vector &stagingPos)
{
	IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
	Assert(pPhys);
...
int	CNPC_Advisor_combat::OnTakeDamage(const CTakeDamageInfo &info)
{
	// Clip our max 
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Advisor_combat::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (flDist > 128)
	{
...
//-----------------------------------------------------------------------------
int CNPC_Advisor_combat::SelectSchedule()
{
	if (IsInAScript())
...
void CNPC_Advisor_combat::HandleAnimEvent(animevent_t *pEvent)
{
	switch (pEvent->event)
	{
	case ADVISOR_MELEE_LEFT:
	{
		CBaseEntity *pHurt = CheckTraceHullAttack(70, -Vector(128, 128, 128), Vector(128, 128, 128), sk_advisor_combat_melee_dmg.GetFloat(), DMG_SLASH);
		if (pHurt)
		{
			Vector forward, up;
			...
			if (pBCC)
			{
				SpawnBlood(pBCC->EyePosition(), g_vecAttackDir, pBCC->BloodColor(), sk_advisor_combat_melee_dmg.GetFloat());
			}
...
//-----------------------------------------------------------------------------
Vector CNPC_Advisor_combat::GetThrowFromPos(CBaseEntity *pEnt)
{
	Assert(pEnt);
	...
	float howFarInFront = advisor_combat_throw_stage_distance.GetFloat() + effecRadius * 1.43f;// clamp(lenToPlayer - posDist + effecRadius,effecRadius*2,90.f + effecRadius);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::Precache()
{
	BaseClass::Precache();

	if (!GetModelName())
	{
		SetModelName(MAKE_STRING("models/advisor.mdl"));
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::IdleSound()
{
	EmitSound("NPC_Advisor.Speak");
}


void CNPC_Advisor_combat::AlertSound()
{
	EmitSound("NPC_Advisor.ScreenVx02");
}


void CNPC_Advisor_combat::PainSound(const CTakeDamageInfo &info)
{
	EmitSound("NPC_Advisor.Pain");
}


void CNPC_Advisor_combat::DeathSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Advisor_combat::DrawDebugTextOverlays()
{
...
//-----------------------------------------------------------------------------
int CNPC_Advisor_combat::GetSoundInterests()
{
...
//-----------------------------------------------------------------------------
bool CNPC_Advisor_combat::QueryHearSound(CSound *pSound)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::InputSetThrowRate(inputdata_t &inputdata)
{
	advisor_combat_throw_rate.SetValue(inputdata.value.Float());
}

//-----------------------------------------------------------------------------
// Sets the number of staging points to levitate the props before being launched
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::InputSetStagingNum(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::InputPinPlayer(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::StopPinPlayer(inputdata_t &inputdata)
{
	m_hPlayerPinPos = NULL;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::OnScheduleChange(void)
{
	Write_AllBeamsOff();
	m_hvStagedEnts.RemoveAll();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::GatherConditions(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::InputWrenchImmediate(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::Write_BeamOn(CBaseEntity *pEnt)
{
	Assert(pEnt);
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::Write_BeamOff(CBaseEntity *pEnt)
{
	Assert(pEnt);
	EntityMessageBegin(this, true);
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::Write_AllBeamsOff(void)
{
	EntityMessageBegin(this, true);
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::InputTurnBeamOn(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_Advisor_combat::InputTurnBeamOff(inputdata_t &inputdata)
{
	// inputdata should specify a target
	CBaseEntity *pTarget = gEntList.FindEntityByName(NULL, inputdata.value.StringID());
	if (pTarget)
...
void CNPC_Advisor_combat::InputElightOn(inputdata_t &inputdata)
{
	EntityMessageBegin(this, true);
...
void CNPC_Advisor_combat::InputElightOff(inputdata_t &inputdata)
{
	EntityMessageBegin(this, true);
...
// MOTION CALLBACK
//==============================================================================================
CAdvisorCombatLevitate::simresult_e	CAdvisorCombatLevitate::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular)
{
	// this function can be optimized to minimize branching if necessary (PPE branch prediction) 
	CNPC_Advisor_combat *pAdvisor = static_cast<CNPC_Advisor_combat *>(m_Advisor.Get());
	Assert(pAdvisor);
...
//-----------------------------------------------------------------------------
const impactdamagetable_t &CNPC_Advisor_combat::GetPhysicsImpactDamageTable(void)
{
	return advisor_combat_use_impact_table.GetBool() ? gAdvisorImpactDamageTable : BaseClass::GetPhysicsImpactDamageTable();
}
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC(npc_advisor, CNPC_Advisor_combat)
