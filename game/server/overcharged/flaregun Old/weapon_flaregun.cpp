...
CFlare::CFlare( void )
{
	...
	iSparkCount		= 0;	// L1ght 15 : Spark count before remove
}
...
{
	//if (m_nBounces > 0) //&& ( GetWaterLevel() < 1 ))		// L1ght 15 : Fixin game loop if we got more that 1 bounce and touch the water
	//{
		/*int contents = UTIL_PointContents ( GetAbsOrigin() );
		if ( (contents & MASK_WATER) || ( GetWaterLevel() < 1 ) )
		{
			//if ( m_nBounces == 0 )
			m_nBounces == -1;
			UTIL_Remove( this );
	...
	if ( ( m_nBounces < 10 ) && ( GetWaterLevel() < 1 ) )
	{
		...
		iSparkCount = iSparkCount + 1;			// L1ght 15 : Stuck flares bug fixing
		if (iSparkCount == 6)
		{
			UTIL_Remove( this );
		}
		//DevMsg("Spark Count: %i \n", iSparkCount);
	}
	...
	if ( pOther && pOther->m_takedamage )
	{
		...
		//=== L1ght 15 ; Dmg is back
		/*if ((pOther && FClassnameIs( pOther, "npc_citizen" ) || FClassnameIs( pOther, "npc_kleiner" ) || FClassnameIs( pOther, "npc_eli" ) || FClassnameIs( pOther, "npc_mossman" )
			|| FClassnameIs( pOther, "npc_alyx" ) || FClassnameIs( pOther, "npc_barney" ) || FClassnameIs( pOther, "npc_breen" ) || FClassnameIs( pOther, "npc_dog" ) || FClassnameIs( pOther, "npc_gman" )
			|| FClassnameIs( pOther, "npc_monk" ) ) && (cvar->FindVar("aa_friendlyfire")->GetInt() == 0))
		{
			//Msg ("FRIENDLY \n");	// take NO damage
		}
		else
		{
			//Msg ("NOT FRIENDLY \n");	// else everybody take dmg
	...
	else
	{
		...
		if (tr.surface.flags & SURF_SKY )	// L1ght 15 : NO MORE STUCK IN THE SKY
			UTIL_Remove( this );
...
acttable_t CFlaregun::m_acttable[] =				// Light Kill : Flaregun act's over here
{
...
void CFlaregun::PrimaryAttack( void )
{
	...
	angles.x += random->RandomFloat(-X, X) - pPlayer->GetPunchAngle().x; //random->RandomInt(-1.1, 1.1);
	angles.y += random->RandomFloat(-Y, Y) - pPlayer->GetPunchAngle().y;//random->RandomInt(-1.1, 1.1);//0.45
	angles.z = random->RandomFloat(-Z, Z);
...
void CFlaregun::SecondaryAttack( void )
{
	...
	angles.x += random->RandomFloat(-1.32, 1.32) - pOwner->GetPunchAngle().x;
	angles.y += random->RandomFloat(-1.32, 1.32) - pOwner->GetPunchAngle().y;
	angles.z = 0;
