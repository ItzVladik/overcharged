...
#include "weapon_m16.h"
// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

//extern ConVar    sk_plr_dmg_m16_grenade;	
ConVar sk_plr_dmg_m16_grenade("sk_plr_dmg_m16_grenade", "0");
...
void CWeaponM16::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch( pEvent->event )
	{
	case EVENT_WEAPON_SMG1:		// BJ: no m16 events declared lol
		{
			Vector vecShootOrigin, vecShootDir;
			QAngle angDiscard;
...
void CWeaponM16::FireNPCSecondaryAttack(CBaseCombatCharacter *pOperator, bool bUseWeaponAngles)
{
...
void CWeaponM16::SecondaryAttack(void)
{
	...
	//MikeD: Moved to BaseClass into GetSecondaryAttackActivity()
	/*if (pPlayer->GetAmmoCount(m_iSecondaryAmmoType) == 1) //&& m_iClip1 <= 0)	//BJ: If alt ammo grenade is LAST, do altfire without reload nade
	{
	//DevMsg("m16 play last grenade animation \n");
	SendWeaponAnim(ACT_VM_EMPTY_FIRE);	// BJ: Short altattack anim
	}
	else*/
	SendWeaponAnim(GetSecondaryAttackActivity());

	// BJ: M16 should load grenade without interrupt, full sequence
