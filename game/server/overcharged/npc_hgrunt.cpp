...
//
// Purpose: This is the soldier version of the HGRUNT, analogous to the HL1 grunt.
//
//=============================================================================//
...
//ConVar sk_HGRUNT_guard_health( "sk_HGRUNT_guard_health", "0");
//ConVar sk_HGRUNT_guard_kick( "sk_HGRUNT_guard_kick", "0");
 
// Whether or not the HGRUNT guard should spawn health on death
//ConVar HGRUNT_guard_spawn_health( "HGRUNT_guard_spawn_health", "1" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the HGRUNT should spawn health on death
ConVar	hgrunt_spawn_health( "hgrunt_spawn_health", "1" );
...
extern Activity ACT_HGRUNT_WALK_EASY;
extern Activity ACT_HGRUNT_WALK_MARCH;
...
void CNPC_Hgrunt::Spawn( void )
{
	...
	// Light Kill : Elite HECU soldiers ? no way !
	/*if( IsElite() )
	{
		// Stronger, tougher.
		SetHealth( sk_HGRUNT_guard_health.GetFloat() );
		SetMaxHealth( sk_HGRUNT_guard_health.GetFloat() );
		SetKickDamage( sk_HGRUNT_guard_kick.GetFloat() );
	}
	else
	{*/
		m_fIsElite = false;
	...
	if (cvar->FindVar("oc_npc_allow_jumping")->GetBool())
		CapabilitiesAdd( bits_CAP_MOVE_JUMP);	//TEST
...
void CNPC_Hgrunt::Precache()
{
	...
	if( !Q_stricmp( pModelName, "models/hgrunt.mdl" ) )
	{
		m_fIsElite = true;
	}
	else
	...
	if( !GetModelName() )
	{
		//SetModelName( MAKE_STRING( "models/HGRUNT_soldier.mdl" ) );
		SetModelName( MAKE_STRING( "models/hgrunt.mdl" ) );
	}
...
void CNPC_Hgrunt::DeathSound( const CTakeDamageInfo &info )
{
	...
	GetSentences()->Speak( "HGRUNT_DIE", SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS ); 
}
...
void CNPC_Hgrunt::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( hgrunt_spawn_health.GetBool() == false || PlayerHasMegaPhysCannon() )
	{
	...
	if ( pPlayer != NULL )
	{
		...
		{
#ifdef HL2_EPISODIC
			if ( HasSpawnFlags( SF_HGRUNT_NO_AR2DROP ) == false )
#endif
			{
		...
		if ( HasSpawnFlags( SF_HGRUNT_NO_GRENADEDROP ) == false )
		{
...
{
	// HGRUNT considers AR2 fire to be heavy damage
...
Activity CNPC_Hgrunt::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	{
		eNewActivity = ACT_HGRUNT_WALK_MARCH;
	}
