...
extern CBaseEntity *CreateRagGib(const char *szModel, const Vector &vecOrigin, const QAngle &vecAngles, const Vector &vecForce, float flFadeTime = 0.0, bool bShouldIgnite = false, int bloodColor = -1);
extern CBaseEntity *CreateRagGibBodygrouped(const char *szModel, const Vector &vecOrigin, const QAngle &vecAngles, const Vector &vecForce, float flFadeTime = 0.0, bool bShouldIgnite = false, int iGroup = 0, int iValue = 0, int bloodColor = -1);
...
public:
	...
	void Spawn(const char *szModel, const Vector &vecOrigin, const Vector &vecForce, float flFadeTime, int bloodColor, bool bShouldIgnite);
	void SpawnBodygrouped(const char *szModel, const Vector &vecOrigin, const Vector &vecForce, float flFadeTime, int iGroup, int iValue, int bloodColor, bool bShouldIgnite);
	void BecomeRagdoll(const CTakeDamageInfo &info, const Vector &forceVector, int bloodColor, bool bShouldIgnite = false);
	void FixupBurningServerRagdoll(CBaseEntity *pRagdoll, bool bShouldIgnite = false);
	void SetupServerRagdoll(CBaseEntity *pRagdoll);
};
