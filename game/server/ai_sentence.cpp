...
void CAI_SentenceBase::UpdateSentenceQueue()
{
	...
	{
		SENTENCEG_PlaySentenceIndex(GetOuter()->edict(), m_nQueuedSentenceIndex, GetVolume(), GetSoundLevel(), 0, GetVoicePitch() * cvar->FindVar("host_timescale")->GetFloat());
...
{
	int nSentenceIndex = SENTENCEG_PlayRndSz(GetOuter()->edict(), pSentence, GetVolume(), GetSoundLevel(), 0, GetVoicePitch() * cvar->FindVar("host_timescale")->GetFloat());
	if ( nSentenceIndex < 0 )
	{
