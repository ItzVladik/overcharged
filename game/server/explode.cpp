...
BEGIN_DATADESC( CEnvExplosion )
	...
	DEFINE_FIELD( m_bSimplefield, FIELD_BOOLEAN ),
...
{ 
	...
	if ( UTIL_PointContents( GetAbsOrigin() ) & CONTENTS_WATER )
	{
		if (m_bSimplefield)
		{
			DispatchParticleEffect((cvar->FindVar("oc_fx_particle_helibomb_explosion_effect_name")->GetString()), vecExplodeOrigin, vec3_angle);
		}
		else
		{
			// draw sparks
...
void ExplosionCreate(const Vector &center, const QAngle &angles,
	CBaseEntity *pOwner, int magnitude, int radius, int nSpawnFlags, bool bSimplefield, float flExplosionForce, CBaseEntity *pInflictor, int iCustomDamageType,
	const EHANDLE *ignoredEntity, Class_T ignoredClass)
{
	...
	pExplosion->m_bSimplefield = bSimplefield;
	...
	CEffectData	data;
	data.m_vOrigin = center;
	data.m_nEntIndex = (pOwner != NULL ? pOwner->entindex() : 0);
...
void ExplosionCreate(const Vector &center, const QAngle &angles,
	CBaseEntity *pOwner, int magnitude, int radius, bool doDamage, bool bSimplefield, float flExplosionForce, bool bSurfaceOnly, bool bSilent, int iCustomDamageType)
{
	// For E3, no sparks
	...
	ExplosionCreate(center, angles, pOwner, magnitude, radius, nFlags, bSimplefield, flExplosionForce, NULL, iCustomDamageType);
}
