...
void CWaterBullet::Touch( CBaseEntity *pOther )
{
	...
	//if (pOther)
	/*{
		DevMsg("Surface: %s", tr.m_pEnt->GetEntityName());
		DevMsg("Hit the surface \n");
		CalculateBulletDamageForce(&info, AmmoType, m_vecDirShooting, tr.endpos);
		info.ScaleDamageForce(flActualForce);
		info.SetAmmoType(AmmoType);
		pOther->DispatchTraceAttack(info, m_vecDirShooting, &tr);
		pOther->TakeDamage(CTakeDamageInfo(this, info.GetInflictor(), flActualDamage, DMG_BULLET));
	}*/
