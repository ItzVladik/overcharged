...
int CAI_PlayerAlly::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	/*if (subInfo.GetDamageType() == DMG_PHYSGUN && cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt())
	...
	{
		if (Classify() == CLASS_PLAYER_ALLY_VITAL && !(subInfo.GetDamageType() & DMG_CRUSH) && !(subInfo.GetDamageType() & DMG_PHYSGUN))// && !cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt())
		{
