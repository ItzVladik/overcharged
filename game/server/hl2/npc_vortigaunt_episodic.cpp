...
void CNPC_Vortigaunt::Spawn( void )
{
	...
	if (FStrEq(STRING(gpGlobals->mapname), "ep1_citadel_00") ||
		FStrEq(STRING(gpGlobals->mapname), "d3_breen_01"))
	{
		SetModelScale(0.0f, 0.0f);

		m_flNextAppearTime = gpGlobals->curtime + 2.3f;
	}
}

void CNPC_Vortigaunt::Think()
{
	if (FStrEq(STRING(gpGlobals->mapname), "ep1_citadel_00") ||
		FStrEq(STRING(gpGlobals->mapname), "d3_breen_01"))
	{
		if (gpGlobals->curtime > m_flNextAppearTime)
			SetModelScale(1.0f, 0.0f);
	}
