...
{ 
	...
	if (!m_iOn)
	{
		...
		//cvar->FindVar("oc_state_RECHARGE")->SetValue(1);
	...
	//te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 255, 200, 180, 0, 150, 0.3, 150);

	CBasePlayer *pl = (CBasePlayer *) m_hActivator.Get();

	if (pl->IsPlayer())
	{
		EntityMessageBegin(pl);
		WRITE_BYTE(PLAYER_MSG_DO_RECHARGE);
		MessageEnd();
	}
	// charge the player
	int nMaxArmor = 100;
...
{
	//cvar->FindVar("oc_state_RECHARGE")->SetValue(0);
	CBasePlayer *player = UTIL_GetLocalPlayer();
	if (player)
	{
		EntityMessageBegin(player);
			WRITE_BYTE(PLAYER_MSG_STOP_RECHARGE);
		MessageEnd();
	}
...
{ 
	...
	if ((m_iOn == 1) && (m_flSoundTime <= gpGlobals->curtime))
	{
		...
		//cvar->FindVar("oc_state_RECHARGE")->SetValue(1);
		//DevMsg("oc_state_RECHARGE: %.2f \n", cvar->FindVar("oc_state_RECHARGE")->GetFloat());
	}
