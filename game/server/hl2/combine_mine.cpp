...
void CBounceBomb::CheckPlrTouch(CBaseEntity *pOther)
{
	if (pOther && pOther->IsPlayer() && IsFriend(pOther))
	{
		CBasePlayer *pPlayer = ToBasePlayer(pOther);
		if (!pPlayer)
			return;

		if (!pPlayer->HasNamedPlayerItem("weapon_hopmine"))
			pPlayer->GiveNamedItem("weapon_hopmine");
		else
		{
			CBaseCombatWeapon *pWeapon = pPlayer->GetWeaponByClassName("weapon_hopmine");
			if (pWeapon)
			{
				...
				if (!g_pGameRules->CanHaveAmmo(pPlayer, iAmmoType))
					...
					pPlayer->GiveAmmo(1, ammoText);
			}
		}

		/*CBaseCombatWeapon *pWeapon = pPlayer->GetWeapon("weapon_hopmine");
		if (pWeapon)
		{
			pWeapon->m_nSkin = this->m_nSkin;
		}*/
...
bool CBounceBomb::IsFriend( CBaseEntity *pEntity )
{
	...
	if (classify == CLASS_ZOMBIE || 
		classify == CLASS_HEADCRAB || 
		classify == CLASS_ANTLION || 
		classify == CLASS_ALIENCONTROLLER ||
		classify == CLASS_ALIENGRUNT || 
		classify == CLASS_BULLSQUID || 
		classify == CLASS_HOUNDEYE || 
		classify == CLASS_HACKED_ROLLERMINE || 
		classify == CLASS_RACEX)
	{
		return false;
	}
  	...
  	if( classify == CLASS_METROPOLICE || 
		...
		classify == CLASS_COMBINEGUARD || 
		classify == CLASS_CREMATOR || 
