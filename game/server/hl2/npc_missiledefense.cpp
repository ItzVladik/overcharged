...
//
// Purpose: Maestro & BriJee better version
//
// $NoKeywords: $
//=============================================================================//
...
{
	PrecacheModel("models/missile_defense.mdl");	// BriJee: Def model
...
void CNPC_MissileDefense::Spawn(void)
{
	...
	if (!GetModel())	// BriJee: No custom model? Use default
	{
...
void CNPC_MissileDefense::Explode(const Vector &vecExplosionPos)
{
	...
	ExplosionCreate(vecExplosionPos, vec3_angle, this, 1000, 500.0f,
		...
		SF_ENVEXPLOSION_NOSMOKE | SF_ENVEXPLOSION_NOFIREBALLSMOKE, false, 0);
