...
ConVar	npc_citizen_auto_player_squad( "npc_citizen_auto_player_squad", "1" , FCVAR_ARCHIVE);
ConVar	npc_citizen_auto_player_squad_allow_use("npc_citizen_auto_player_squad_allow_use", "0" , FCVAR_ARCHIVE);
	...
	{
		{
			...
			AI_TraceHull(GetAbsOrigin() + Vector(0, 0, 15), GetAbsOrigin() + Vector(0, 0, 15) + vecDir + Vector(0, 0, -15), GetHullMins(), GetHullMaxs(), MASK_SHOT, this, GetCollisionGroup(), &tr);

			NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vecDir, 0, 255, 0, false, 2.0);

			if (!tr.DidHitWorld() && (GetFlags() & FL_ONGROUND))
			{
				...
				Vector m_vecJumpVel = moveTrace.vJumpVelocity;
				...
				if (!IsMoveBlocked(moveTrace))
				{
					...
					if (fJumpIsLeft)
					{
						SetIdealActivity((Activity)ACT_JUMP);
						GetNavigator()->SetMovementActivity((Activity)ACT_JUMP);
					}
					else
					{
						SetIdealActivity((Activity)ACT_JUMP);
