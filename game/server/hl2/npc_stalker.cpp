...
BEGIN_DATADESC( CNPC_Stalker )
	...
	DEFINE_KEYFIELD(m_iAngryOnSpawn, FIELD_INTEGER, "angryonspawn"),	// BJ: Always angry on spawn new flag
...
int	CNPC_Stalker::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	// --------------------------------------------
	if (info.GetAttacker() && info.GetAttacker()->Classify() == CLASS_VORTIGAUNT)
	{
	...
	{
		if (m_iAngryOnSpawn == 0)		// BJ: Always angry on spawn new flag
...
void CNPC_Stalker::Spawn( void )
{
	...
	{
		AddClassRelationship(CLASS_PLAYER, D_HT, 9999);
		m_iPlayerAggression = 999;
	}
...
void CNPC_Stalker::Precache( void )
{
	...
	PrecacheModel(STRING(GetModelName()));		// Allow custom model usage
...
{
	if (IsInSquad() && info.GetAttacker() && info.GetAttacker()->IsPlayer())
	{
		AISquadIter_t iter;
