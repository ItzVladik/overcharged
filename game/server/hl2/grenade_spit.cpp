...
void CGrenadeSpit::GrenadeSpitTouch( CBaseEntity *pOther )
{
	...
	{
		// Make a splat decal
		UTIL_DecalTrace(pTrace, "BeerSplash");
	}
	...
	if ( pTraceEnt )
	{
		...
		RadiusDamage(CTakeDamageInfo(this, GetThrower(), m_flDamage * poisonratio, (DMG_ACID)), pTrace->endpos, m_flDamage * poisonratio, CLASS_NONE, GetThrower());

		//CNPC_Antlion *pAnt = static_cast<CNPC_Antlion*>(GetThrower());

		/*if (GetThrower() && pTraceEnt->IsPlayer() && (GetThrower()->IRelationType(pTraceEnt) == D_LI) || (GetThrower()->IRelationType(pTraceEnt) == D_NU))
		{
			//None
		}
		else
