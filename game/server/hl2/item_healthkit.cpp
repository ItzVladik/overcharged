...
void CHealthKit::Spawn( void )
{
	...
	this->AddEffects(EF_HEALTHLIGHT);
	BaseClass::Spawn();
...
bool CHealthKit::MyTouch( CBasePlayer *pPlayer )
{
	if ( pPlayer->TakeHealth( sk_healthkit.GetFloat(), DMG_GENERIC ) )
	{
		if (pPlayer->IsPlayer())
		{
			EntityMessageBegin(pPlayer);
				WRITE_BYTE(PLAYER_MSG_DO_HEALTH);
			MessageEnd();
		}

		pPlayer->CanDropGarbage = true;
		pPlayer->garbage.AddToHead(MAKE_STRING("item_healthkit"));
		pPlayer->garbageIndex.AddToHead(gpGlobals->curtime + cvar->FindVar("oc_player_garbage_drop_delay")->GetFloat());
...
public:
	...
	bool MyTouch( CBasePlayer *pPlayer )
	{
		if ( pPlayer->TakeHealth( sk_healthvial.GetFloat(), DMG_GENERIC ) )
		{
			...
			pPlayer->CanDropGarbage = true;
			pPlayer->garbage.AddToHead(MAKE_STRING("item_healthvial"));
			pPlayer->garbageIndex.AddToHead(gpGlobals->curtime + cvar->FindVar("oc_player_garbage_drop_delay")->GetFloat());
...
{ 
	...
	{
		EntityMessageBegin(pPlayer);
			WRITE_BYTE(PLAYER_MSG_DO_HEALTH_REGEN);
		MessageEnd();
	}
...
void CWallHealth::Off(void)
{
	...
	if (player)
	{
		EntityMessageBegin(player);
			WRITE_BYTE(PLAYER_MSG_STOP_HEALTH_REGEN);
		MessageEnd();
	}
	// Stop looping sound.
	if (m_iOn > 1)
