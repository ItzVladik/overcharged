...
void CNPC_Alyx::DoCustomSpeechAI( void )
{
	...
	if ( HasCondition(COND_NEW_ENEMY) && GetEnemy() )
	{
		...
		else if ( GetEnemy()->Classify() == CLASS_ZOMBIE ) 
		{
			...
			if (!FStrEq(pZombie->GetClassname(), "npc_gonome"))
			{
...
{
	if (cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt() && info.GetAttacker() && info.GetAttacker()->IsPlayer())
	{
...
/*	float fBlindDist = ( m_bIsFlashlightBlind ) ? 35.0f : 30.0f;
	...
	CFlare *pFlare = CFlare::GetActiveFlares();//CRASH ON EP 1
