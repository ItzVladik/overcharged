...
void CProtoSniper::Event_Killed( const CTakeDamageInfo &info )
{
	...
	{
		Vector vecForward, Up;
		
		float flForce = random->RandomFloat( 500, 700 ) * 10;

		AngleVectors( GetLocalAngles(), &vecForward, NULL, &Up );
		...
		pRagdoll->SetLocalOrigin(GetLocalOrigin());
		pRagdoll->SetLocalAngles(GetLocalAngles());
		pRagdoll->SetLocalVelocity((vecForward * flForce) + Vector(0, 0, 600));*/
		
		CBaseEntity *pW = static_cast<CBaseEntity*>(CreateEntityByName("weapon_combinesniper"));
		//pW = CreateEntityByName("weapon_sniper");
		pW->SetMoveType(MOVETYPE_VPHYSICS);
		pW->SetCollisionGroup(COLLISION_GROUP_WEAPON);
		pW->SetLocalOrigin(GetLocalOrigin());
		pW->SetLocalAngles(GetLocalAngles());
		pW->Spawn();
		//pW->SetLocalVelocity((vecForward * flForce) + Vector(0, 0, 600));


		pW->ApplyAbsVelocityImpulse((vecForward * (flForce*0.2f)) + Vector(0, 0, 600));//(vecForward * 300 + Vector(0, 0, 400));
		//pW->SetAbsVelocity((vecForward * flForce) + Vector(0, 0, 600));//((vecForward * flForce*38) + Vector(0, 0, 10600));

		/*pW->SetMoveType(MOVETYPE_VPHYSICS);
		pW->SetCollisionGroup(COLLISION_GROUP_DEBRIS);*/
	}
