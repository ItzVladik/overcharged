...
void CPropCombineBall::ReadDataFromFile(IFileSystem* filesystem, CBaseEntity *pBall)
{
	KeyValues *pkvProperties = new KeyValues("prop_combine_ball");
	char script[256];
	m_Particles = false;
	Q_snprintf(script, sizeof(script), "scripts/entites/%s.txt", "prop_combine_ball");
	if (pkvProperties->LoadFromFile(filesystem, "scripts/entites/prop_combine_ball.txt", "GAME"))
	{
		if (pkvProperties)
		{
...
void CPropCombineBall::Spawn( void )
{
	...
	if (m_Particles)
		DispatchParticleEffect(pTrail, PATTACH_ABSORIGIN_FOLLOW, this);
}
...
void CPropCombineBall::DoExplosion( )
{
	...
	if (m_Particles)
		DispatchParticleEffect(pExplode, GetAbsOrigin(), GetAbsAngles(), this);
