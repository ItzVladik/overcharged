...
ConVar oc_weapon_megaphyscannon_use_battery("oc_weapon_megaphyscannon_use_battery", "0", FCVAR_ARCHIVE);
ConVar oc_weapon_megaphyscannon_time_to_remove_battery("oc_weapon_megaphyscannon_time_to_remove_battery", "200", FCVAR_ARCHIVE);
...
#define	MEGACANNON_MODEL /*GetOwner()->GetActiveWeapon()->GetWpnData().szAdditionalViewModel*/"models/weapons/v_superphyscannon.mdl"
...
//	ComputeMaxSpeed( pEntity, pPhys );
	...
	if (dynamic_cast<CRagdollProp*>(pEntity) 
		&& !FClassnameIs(pEntity, "npc_rollermine")
		&& !FClassnameIs(pEntity, "npc_manhack")
		&& !FClassnameIs(pEntity, "npc_scanner")
		&& !FClassnameIs(pEntity, "npc_turret_floor")
		&& !FClassnameIs(pEntity, "npc_antlionguard"))
	{
		//DevMsg("AttachEntity \n");
...
void CPlayerPickupController::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( ToBasePlayer(pActivator) == m_pPlayer )
	{
		...
		{
			//if (cvar->FindVar("oc_single_primary_weapon_mode")->GetInt() == 1)			//===========BRIJEE OVR: Not oblivious crash in this mode. Fixed.
			{
				//if (m_pPlayer->GetUseEntity->pWeapon)//GetActiveWeapon()->Deploy())
				
				if (pAttached->IsBaseCombatWeapon())
				{
					//DevMsg("WEAPON DETECTED \n");
					Shutdown();
					return;
				}
			}//===========================END
...
public:
	...
	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
	DECLARE_ACTTABLE();			// BriJee OVR: Player anims.
	...
	void	WeaponIdle();
	void	WeaponBecomeMega(CBasePlayer *pPlayer);
	void	ItemPreFrame();
	void	ItemPostFrame();
	void	ItemBusyFrame();
...
BEGIN_DATADESC( CWeaponPhysCannon )
	...
	DEFINE_FIELD(isMegaPhys, FIELD_BOOLEAN),

	DEFINE_FIELD(timeToRemoveBattery, FIELD_INTEGER),
	DEFINE_FIELD(timeToSelfShock, FIELD_INTEGER),
...
//========================BriJee Player animations definition.
...
void CWeaponPhysCannon::Precache( void )
{
	...
	//PrecacheModel(GetWpnData().szAdditionalViewModel);

	PrecacheScriptSound( "Weapon_PhysCannon.HoldSound" );
	PrecacheScriptSound( "Weapon_Physgun.Off" );
	PrecacheScriptSound("Weapon_physcannon.Activate");
	PrecacheScriptSound("Weapon_physcannon.Deactivate");
...
bool CWeaponPhysCannon::Deploy( void )
{
	...
	if (pPlayer != NULL)
	{
		if ((pPlayer->ArmorValue() <= 0) && (oc_weapon_megaphyscannon_use_battery.GetBool()))
		{
			if (isMegaPhys || cvar->FindVar("physcannon_mega_enabled")->GetBool())
			{
				isMegaPhys = false;
				cvar->FindVar("physcannon_mega_enabled")->SetValue(0);
				timeToRemoveBattery = 0;
			}
		}
		else if (!oc_weapon_megaphyscannon_use_battery.GetBool())
		{
			if (!isMegaPhys && cvar->FindVar("physcannon_mega_enabled")->GetBool())
			{
				cvar->FindVar("physcannon_mega_enabled")->SetValue(0);
	...
	CloseElements();
	//if (cvar->FindVar("oc_weapon_holster")->GetInt() == 0)		// BriJee OVR: Best fix of phys deploy effects bug in new holster system
	{
...
void CWeaponPhysCannon::SetViewModel( void )
{
	...
	char emp[2] = "";
	if (pOwner && pOwner->GetActiveWeapon() && pOwner->GetActiveWeapon()->GetWpnData().szAdditionalViewModel != NULL && pOwner->GetActiveWeapon()->GetWpnData().szAdditionalViewModel != emp)
	{
		vm->PrecacheModel(GetWpnData().szAdditionalViewModel);
		vm->SetWeaponModel(pOwner->GetActiveWeapon()->GetWpnData().szAdditionalViewModel, pOwner->GetActiveWeapon());
	}
...
void CWeaponPhysCannon::Drop( const Vector &vecVelocity )
{
	...
	if (cvar->FindVar("physcannon_mega_enabled")->GetBool() && isMegaPhys)
	{
		isMegaPhys = false;
		cvar->FindVar("physcannon_mega_enabled")->SetValue(0);
		EmitSound("Weapon_physcannon.Deactivate");
		timeToRemoveBattery = 0;
	}
...
bool CWeaponPhysCannon::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	...
	timeToSelfShock = 0;
	DestroyEffects();
	UTIL_Remove(m_hGlowSprites[NUM_SPRITES]);
	UTIL_Remove(m_hEndSprites[2]);
...
void CWeaponPhysCannon::DryFire( void )
{
	...
	pOwner->SetAnimation(PLAYER_ATTACK1);				// BriJee OVR: Player anims.
}
...
void CWeaponPhysCannon::PrimaryFireEffect( void )
{
	...
	pOwner->ViewPunch(QAngle(X, random->RandomInt(-2, 2), 0));
...
void CWeaponPhysCannon::PuntRagdoll( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	...
	{
		info.SetDamageType(DMG_REMOVENORAGDOLL);
	}
	pEntity->DispatchTraceAttack( info, forward, &tr );
	ApplyMultiDamage();
...
void CWeaponPhysCannon::PrimaryAttack( void )
{
	...
	if ( pEntity->GetMoveType() != MOVETYPE_VPHYSICS )
	{
		...
		if( GetOwner()->IsPlayer() && !IsMegaPhysCannon() )
		{
			...
			/*if( pEntity->IsNPC() && pEntity->Classify() != CLASS_HEADCRAB && !FClassnameIs(pEntity, "npc_antlion") )//OVR FUNNY FIX
			{
				DryFire();
				return;
			}*/
			...
			&& !FClassnameIs(pEntity, "npc_antlionguard"))
		{
			if ( pEntity->IsNPC() && !pEntity->IsEFlagSet( EFL_NO_MEGAPHYSCANNON_RAGDOLL ) && pEntity->MyNPCPointer()->CanBecomeRagdoll() )
			{
				...
				if (pEntity->MyNPCPointer()->IsPlayerAlly() && !cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt())
					return;

				//CTakeDamageInfo info( pOwner, pOwner, 1.0f, DMG_GENERIC );
				int damage = /*(pEntity->Classify() == CLASS_PLAYER_ALLY || pEntity->Classify() == CLASS_PLAYER_ALLY_VITAL) ? pEntity->GetHealth() * 4 : */pEntity->GetHealth();
				//DevMsg("damage %i \n", damage);
				CTakeDamageInfo info(pOwner, pOwner, damage, DMG_GENERIC);

				//pEntity->MyNPCPointer()->Event_Killed(info);

				CBaseEntity *pRagdoll = CreateServerRagdoll(pEntity->MyNPCPointer(), 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, true, pEntity->MyNPCPointer()->BloodColor());
				...
				CTakeDamageInfo ragdollInfo(pOwner, pOwner, damage, DMG_PHYSGUN | DMG_REMOVENORAGDOLL);
				pEntity->TakeDamage( ragdollInfo );

				if (pEntity->IsNPC())
					pEntity->Event_Killed(ragdollInfo);
	...
	else
	{
		...
		if ( !IsMegaPhysCannon() )
		{
			/*if ( pEntity->VPhysicsIsFlesh( ) )//OVR FUNNY FIX
			{
				DryFire();
				return;
			}*/
		...
		{
			if (FClassnameIs(pEntity, "npc_turret_floor") 
				|| FClassnameIs(pEntity, "npc_rollermine")
				|| FClassnameIs(pEntity, "npc_manhack")
				|| FClassnameIs(pEntity, "npc_scanner")
				|| FClassnameIs(pEntity, "npc_antlionguard"))
			...
			else
			{
				if (dynamic_cast<CRagdollProp*>(pEntity))
				{
					DevMsg("PrimaryAttack PuntRagdoll \n");
...
{
	if (GetWpnData().animData[m_bFireMode].RunAnimation || GetWpnData().animData[m_bFireMode].WalkAnimation || GetWpnData().animData[m_bFireMode].WallAnimation)
...
bool CWeaponPhysCannon::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
	...
	if (CanPickupObject(pObject) == false || FClassnameIs(pObject, "prop_vehicle_jeep") || FClassnameIs(pObject, "npc_apcdriver") || FClassnameIs(pObject, "prop_vehicle_apc"))
		return false;
	...
	{
		if (pObject && pObject->IsNPC() && !pObject->IsEFlagSet(EFL_NO_MEGAPHYSCANNON_RAGDOLL) && !FClassnameIs(pObject, "npc_rollermine")
			&& !FClassnameIs(pObject, "npc_manhack")
			&& !FClassnameIs(pObject, "npc_scanner")
			&& !FClassnameIs(pObject, "npc_turret_floor")
			&& !FClassnameIs(pObject, "npc_antlionguard"))
		{

			if (pObject->MyNPCPointer()->IsPlayerAlly() && !cvar->FindVar("oc_ally_allow_friendlyfire")->GetInt())
				return false;

			Assert( pObject->MyNPCPointer()->CanBecomeRagdoll() );
			//CTakeDamageInfo info( GetOwner(), GetOwner(), 1.0f, DMG_GENERIC );
			int damage = /*(pObject->Classify() == CLASS_PLAYER_ALLY || pObject->Classify() == CLASS_PLAYER_ALLY_VITAL) ? pObject->GetHealth() * 4 : */pObject->GetHealth();
			//DevMsg("damage %i \n", damage);
			CTakeDamageInfo info(GetOwner(), GetOwner(), damage, DMG_GENERIC);
			//pObject->MyNPCPointer()->Event_Killed(info);

			CBaseEntity *pRagdoll = CreateServerRagdoll(pObject->MyNPCPointer(), 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, true, pObject->MyNPCPointer()->BloodColor());
			PhysSetEntityGameFlags( pRagdoll, FVPHYSICS_NO_SELF_COLLISIONS );
			...
			CTakeDamageInfo ragdollInfo(GetOwner(), GetOwner(), damage, DMG_PHYSGUN | DMG_REMOVENORAGDOLL);
			pObject->TakeDamage( ragdollInfo );

			if (pObject->IsNPC())
				pObject->Event_Killed(ragdollInfo);
...
void CWeaponPhysCannon::DoEffectIdle( void )
{
	...
	if ( m_bPhyscannonState != IsMegaPhysCannon() )
	{
		...
		if ( m_bPhyscannonState == false )
		{
			...
			if (pPlayer /*&& pPlayer->ArmorValue() > 0 */&& !isMegaPhys && oc_weapon_megaphyscannon_use_battery.GetInt() == 2)
			{
	...
	if ( IsMegaPhysCannon() )
	{
		...
		/*if (!engine->IsPaused() && m_EffectState != EFFECT_HOLDING)
		{
			CEffectData data;
			data.m_vOrigin = Muzzle;
			DispatchEffect("MegaPhyscannonLight", data);
			DispatchParticleEffect("weapon_megaphyscannon_core", PATTACH_POINT_FOLLOW, pOwner->GetViewModel(), "muzzle", false);
		}*/
		...
		if (!engine->IsPaused() && m_EffectState == EFFECT_HOLDING)
		{
			for (int i = 0; i < 3; i++)
			{
				...
				DispatchEffect("MegaPhyscannonBeams", data);
			}
#if 0
			DispatchParticleEffect("weapon_megaphyscannon_core", PATTACH_POINT_FOLLOW, pOwner->GetViewModel(), "muzzle", false);
#endif
		}
		if (!engine->IsPaused() && m_EffectState == EFFECT_LAUNCH && m_EffectState != EFFECT_HOLDING)
		{
			for (int i = 0; i <= 5; i++)
			{
				CEffectData	data;
	...
	else
	{
		...
		if (!engine->IsPaused() && m_EffectState != EFFECT_HOLDING)
		{
			...
			DispatchEffect("PhyscannonLight", data);
#if 0
			DispatchParticleEffect("weapon_physcannon_core", PATTACH_POINT_FOLLOW, pOwner->GetViewModel(), "muzzle", false);
#endif
		}
		if (!engine->IsPaused() && m_EffectState == EFFECT_HOLDING)
		{
			for (int i = 0; i < 3; i++)
			{
				...
				//data.m_nAttachmentIndex = 1;
				data.m_vOrigin = Muzzle;
				DispatchEffect("PhyscannonLightIncreased", data);
			}
#if 0
			DispatchParticleEffect("weapon_physcannon_core", PATTACH_POINT_FOLLOW, pOwner->GetViewModel(), "muzzle", false);
#endif
...
void CWeaponPhysCannon::ItemPostFrame()
{
	...
	//CheckAdmireAnimations(pPlayer);

	/*if (pPlayer)
	{
		if (MuzzleFlashTime <= FlashLifeTime())
			MuzzleFlashTime = MuzzleFlashTime + 0.01f; //Увеличиваем переменную на единицу, каждый кадр в секунду
	...
	/*if (IsMegaPhysCannon() && pOwner && m_hCenterSprite != NULL)
	{
		for (int i = 0; i < 3; i++)
		{
			Vector forward, right, up;
			...
			float	testLength = 1.5f;
			Vector	end = start + forward * testLength;
			end += right * RandomVector(-5, 5);
			end += up * RandomVector(-5, 5);

			CEffectData	data;
			data.m_nEntIndex = pOwner->GetViewModel()->entindex();
			data.m_nAttachmentIndex = LookupAttachment("muzzle");
			data.m_vStart = end;//pOwner->GetAutoaimVector(AUTOAIM_SCALE_DEFAULT);
			DispatchEffect("MegaPhyscannonBeams", data);
		}
	}*/
	...
	bool forcedUnavaiable = FStrEq(gpGlobals->mapname.ToCStr(), "d3_citadel_03") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "d3_citadel_04") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "d3_citadel_05") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_00") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_00_demo") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_01") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_02") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_02b") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_03") ||
		FStrEq(gpGlobals->mapname.ToCStr(), "ep1_citadel_04");

	if (!forcedUnavaiable)
	{
		if (isMegaPhys != cvar->FindVar("physcannon_mega_enabled")->GetBool())
			cvar->FindVar("physcannon_mega_enabled")->SetValue((int)isMegaPhys);

		if (oc_weapon_megaphyscannon_use_battery.GetInt())
		{
			if (pOwner->m_afButtonPressed & IN_SWITCHMODE && pPlayer->ArmorValue() > 0)
			{
				WeaponBecomeMega(pPlayer);
			}
			if (isMegaPhys)
			{
				if (timeToRemoveBattery <= oc_weapon_megaphyscannon_time_to_remove_battery.GetInt())
					timeToRemoveBattery++;

				if (timeToRemoveBattery >= oc_weapon_megaphyscannon_time_to_remove_battery.GetInt())
				{
					pPlayer->SetArmorValue(pPlayer->ArmorValue() - 1);
					timeToRemoveBattery = 0;
				}
			}
			if (isMegaPhys && pPlayer->ArmorValue() <= 0 && cvar->FindVar("physcannon_mega_enabled")->GetBool())
			{
				WeaponBecomeMega(pPlayer);
				timeToRemoveBattery = 0;
			}
		...
		{
			if (pOwner->m_afButtonPressed & IN_SWITCHMODE)
			{
				WeaponBecomeMega(pPlayer);
			}
		}
	...
	if (isMegaPhys && pPlayer->GetWaterLevel() == 3)
	{
		if (timeToSelfShock <= 40)
			timeToSelfShock++;
		//else
			//timeToSelfShock = 0;
		...
		if (timeToSelfShock >= 40)
		{
			pOwner->TakeDamage(CTakeDamageInfo(this, this, 10, DMG_GAUSS | DMG_CRUSH));
			EmitSound("Weapon_physcannon.Activate");
			timeToSelfShock = 0;
		}
	}
	else if (isMegaPhys && pPlayer->GetWaterLevel() != 3)
		timeToSelfShock = 0;
...
void CWeaponPhysCannon::WeaponBecomeMega(CBasePlayer *pPlayer)
{

	if (!cvar->FindVar("physcannon_mega_enabled")->GetBool())
	{
		isMegaPhys = true;
		...
		EmitSound("Weapon_physcannon.Activate");
		cvar->FindVar("physcannon_mega_enabled")->SetValue(1);
		timeToRemoveBattery = 0;
		return;
	}
	else if (cvar->FindVar("physcannon_mega_enabled")->GetBool())
	{
		isMegaPhys = false;
		cvar->FindVar("physcannon_mega_enabled")->SetValue(0);
		EmitSound("Weapon_physcannon.Deactivate");
...
bool CWeaponPhysCannon::CanPickupObject( CBaseEntity *pTarget )
{
	...
	if ( !IsMegaPhysCannon() )
	{
		//if ( pTarget->VPhysicsIsFlesh( ) )//OVR FUNNY FIX
			//return false;
		return CBasePlayer::CanPickupObject( pTarget, physcannon_maxmass.GetFloat(), 0 );
	}
	else
	{
		if (FClassnameIs(pTarget, "npc_turret_floor")
			|| FClassnameIs(pTarget, "npc_rollermine")
			|| FClassnameIs(pTarget, "npc_manhack")
			|| FClassnameIs(pTarget, "npc_scanner")
			|| FClassnameIs(pTarget, "npc_antlionguard"))
		{
			return CBasePlayer::CanPickupObject(pTarget, physcannon_maxmass.GetFloat(), 0);
		}
	}
