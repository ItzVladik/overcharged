...
bool CPointTemplate::KeyValue(const char *szKeyName, const char *szValue)
{
	CBaseEntity	*pEntity = NULL;

	for (int i = 0; i < m_hTemplateEntities.Count(); i++)
	{
		if (FStrEq(STRING(m_hTemplateEntities.Element(i)->GetEntityName()), szKeyName))
		{
			pEntity = m_hTemplateEntities.Element(i);
			if (pEntity)
			{
				CUtlVector<char*, CUtlMemory<char*> > arr;
				Q_SplitString(szValue, ":", arr);
				
				if (arr.Count())
					pEntity->KeyValue(arr.Element(0), arr.Element(1));

				break;
			}
		}

		if (FStrEq(m_hTemplateEntities.Element(i)->GetClassname(), szKeyName))
		{
			pEntity = m_hTemplateEntities.Element(i);
			if (pEntity)
			{
...
CBaseEntity *CPointTemplate::GetTemplateByName(const char* pName)
{
	CBaseEntity	*pEntity = NULL;

	for (int i = 0; i < m_hTemplateEntities.Count(); i++)
	{
		if (m_hTemplateEntities.Element(i)->NameMatches(pName))
			pEntity = m_hTemplateEntities.Element(i);	
	}
