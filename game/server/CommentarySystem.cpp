...
void CC_CommentaryFalse(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	ConVarRef var(pConVar);
	var.SetValue(false);
}
//MikeD - we don't need commentary any more...
//ConVar commentary("commentary", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX, "Desired commentary mode state.", CC_CommentaryChanged );
ConVar commentary("commentary", "0", FCVAR_HIDDEN, "Desired commentary mode state.", CC_CommentaryFalse);
