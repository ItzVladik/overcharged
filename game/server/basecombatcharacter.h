...
public:
	...
	// Ammo
	// -----------------------
	virtual int			GiveAmmo( int iCount, int iAmmoIndex, bool bSuppressSound = false, bool refreshGrenade = false );
	...
	virtual void		Weapon_HandleAnimEvent( animevent_t *pEvent, bool resetLoopingSound = false );
	virtual void		NPCPrimaryAttack(animevent_t *pEvent);
	...
	bool				Owner_HasFreeSlots(CBaseCombatWeapon* pWeapon);
...
protected:
	...
	bool					m_bSpawnNoRagdoll;
	void					CharacterEmitSound(const char *m_sName, const Vector &m_vOrigin, int m_eChannel);
...
public:
	...
	CBaseCombatWeapon*	GetWeapon( const char *sWeapon);
...
private:
	...
	void ThrowDirForWeaponStrip( const Vector &vecForward, Vector *pVecThrowDir );
...
protected:
	...
	friend class CShowWeapon;	// BriJee OVR: Weapon holster anim.
