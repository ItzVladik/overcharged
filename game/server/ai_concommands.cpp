	...
	if (baseNPC)
	{
		...
		if ( tr.fraction != 1.0)
		{
			...
			if (baseNPC->CapabilitiesGet() & bits_CAP_MOVE_FLY)
			{
				...
				if (!Q_stricmp(args[2], "plrang"))
				{
					QAngle plrAng = baseNPC->GetAbsAngles();
					plrAng.x = pPlayer->GetAbsAngles().x;

					baseNPC->Teleport(&pos, &plrAng, NULL);
				}
				else if (!Q_stricmp(args[2], "plrrevang"))
				{
					QAngle plrAng = baseNPC->GetAbsAngles();
			...
			else
			{
				...
				if (!Q_stricmp(args[2], "plrang") && args[3] != NULL)
				{
					...
					tr.endpos.z += 12;
					baseNPC->Teleport(&tr.endpos, &plrAng, NULL);
