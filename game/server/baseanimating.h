...
public:
	...
	//void	SetSequenceCycleRate(CStudioHdr *pStudioHdr, int iSequence, int fps);
	//inline void	SetSequenceCycleRate(int iSequence, int fps) { return SetSequenceCycleRate(GetModelPtr(), iSequence, fps); }
	...
	virtual bool IsValidRagdollName(void);
...
#ifndef CLIENT_DLL
	virtual void SetBoneTransform(int iBone, const Vector &in);
#endif
...
// Controllers.
	...
	void DoMuzzleFlash();
	void DoMuzzleFlashOVR(int attachmentIndex, int flashType, int r, int g, int b, const char *name);
	// Fire
	...
	bool Dissolve_Egon(const char *pMaterialName, float flStartTime, bool bNPCOnly = true, int nDissolveType = 0, Vector vDissolverOrigin = vec3_origin, int iMagnitude = 0);
	//virtual void        CalcFirstPersonDeathViewServer(Vector& eyeOrigin, QAngle& eyeAngles, float& fov);		// OVR: 1st person camera attachment
...
public:
	...
	bool				m_bIsBodygrouped = false;
	int					m_iBodyGroup = 0;
	int					m_iBodyGroupValue = 0;
