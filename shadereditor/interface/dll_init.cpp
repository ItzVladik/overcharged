...
IUniformRandomStream *vrandom = NULL;
								...
								bool bCreateEditor, bool bEnablePrimaryDebug, int iSkymaskMode )
{
	...
	if ( (filesystem = (IFileSystem *)appSystemFactory(FILESYSTEM_INTERFACE_VERSION, NULL)) == NULL )
		return false;
#endif
	if ( (vrandom = (IUniformRandomStream *)appSystemFactory(VENGINE_CLIENT_RANDOM_INTERFACE_VERSION, NULL)) == NULL )
		return false;
	...
	s_GaussianRandomStream.AttachToStream( vrandom );
	...
	#else
		const char *pszModuleName = "libgame_shader_dx6.so";
	#endif
...
//	Q_snprintf( modulePath, sizeof( modulePath ), "%s/bin/%s", engine->GetGameDirectory(), pszModuleName );
	ProcShaderModule = Sys_LoadModule( pszModuleName );
	if ( ProcShaderModule )
	{
